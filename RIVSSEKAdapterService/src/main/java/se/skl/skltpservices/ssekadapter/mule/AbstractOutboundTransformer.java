...
import static se.skl.skltpservices.ssekadapter.mapper.AbstractMapper.getInstance;
import static se.skl.skltpservices.ssekadapter.mule.OutboundPreProcessor.*;

/**
 * Abstracts outbound transformers. <p/>
 *
 * Provides a method to lookup a valid se.skl.skltpservices.ssekadapter.mapper.Mapper implementation.
 *
 * @author Peter
 */
public abstract class AbstractOutboundTransformer extends AbstractMessageTransformer {
    ...
    protected Mapper getMapper(final MuleMessage message) throws RouteException {
        ...
        final String targetNSURI = targetNS.getNamespaceURI();
        final Mapper mapper =  getInstance(sourceNS, targetNSURI);
        if (mapper == null) {
